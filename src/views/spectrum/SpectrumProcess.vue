<template>
    <div class="process-panel" :class="{ hidden: isHidden }">
      <div class="main-panel">
        ...
        <div id="picture" class="spec-image" ref="specImageRef" style="width: 900px;height:500px;"></div>

        <div class="res-panel">
          <el-popconfirm title="返回上一步将清空本次所有的处理操作，确定是否继续"  @confirm="toForwoard">
            <template #reference>
...
const doSubmit = () => {
  proFormDataRef.value.validate((valid) => {
      ...
      if(!preProcessMethodList.value.includes(specMothod.value) && specMothod.value != '特征波长提取') {
...
const convertDivToImage = () => {
  return new Promise((resolve, reject) => {
    if (specImageRef.value) {
      html2canvas(specImageRef.value).then((canvas) => {
